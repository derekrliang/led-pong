//FIRST HIGH PINS OF PORT USED FOR OUTPUT
//YOU MUST USE
//PORTA = DATA PINS (a)
//PORTB = CLOCK PINS (c)
//(x,y)=(0,0) is top left
//(x,y)=(31,15) bottom right
...
// connector
// r=red, b=blue, g=green, u=ground, a=row address pins
// c=clock, l=data latch, e=output enable
//
// wire colors
//   notch
// rbrbaace
...
//you should refer to the "ATM32 API PIN" # when referencing the pins
...
void RGBmatrixPanel_init(RGBmatrixPanel *rgbmp, uint8_t rows, uint8_t a, uint8_t b, uint8_t c,
  ...
  rgbmp->_nRows = rows; // Number of multiplexed rows; actual height is 2X this
  Adafruit_GFX_constructor(&rgbmp->agfx, 32, rgbmp->_nRows * 2);
  rgbmp->agfx.drawPixel = &RGBmatrixPanel_drawPixel; //overload
  rgbmp->agfx.invertDisplay = &RGBmatrixPanel_invertDisplay; //overload
  rgbmp->agfx.fillScreen = &RGBmatrixPanel_fillScreen; //overload

  // Allocate and initialize matrix buffer:
  int buffsize  = 32 * rgbmp->_nRows * 3, // x3 = 3 bytes holds 4 planes "packed"
      allocsize = (dbuf == 1) ? (buffsize * 2) : buffsize;
  if(NULL == (rgbmp->_matrixbuff[0] = (uint8_t *)malloc(allocsize))) return;
  memset(rgbmp->_matrixbuff[0], 0, allocsize);
  // If not double-buffered, both buffers then point to the same address:
  rgbmp->_matrixbuff[1] = (dbuf == 1) ? &rgbmp->_matrixbuff[0][buffsize] : rgbmp->_matrixbuff[0];
  ...
  rgbmp->_sclkpin   = digitalPinToBitMask(sclk);
  rgbmp->_latport   = portOutputRegister(digitalPinToPort(latch));
  rgbmp->_latpin    = digitalPinToBitMask(latch);
  rgbmp->_oeport    = portOutputRegister(digitalPinToPort(oe));
  rgbmp->_oepin     = digitalPinToBitMask(oe);
  rgbmp->_addraport = portOutputRegister(digitalPinToPort(a));
  rgbmp->_addrapin  = digitalPinToBitMask(a);
  rgbmp->_addrbport = portOutputRegister(digitalPinToPort(b));
  rgbmp->_addrbpin  = digitalPinToBitMask(b);
  rgbmp->_addrcport = portOutputRegister(digitalPinToPort(c));
  rgbmp->_addrcpin  = digitalPinToBitMask(c); 
  rgbmp->_nPlanes   = 4;     // Other code is fixed at 4 planes; don't change this
  rgbmp->_plane     = rgbmp->_nPlanes - 1;
  rgbmp->_row       = rgbmp->_nRows   - 1;
  rgbmp->_swapflag  = 0;
  rgbmp->_backindex = 0;     // Array index of back buffer
}

// Constructor for 16x32 panel:
void RGBmatrixPanel_Constructor_16x32(RGBmatrixPanel *rgbmp, 
  uint8_t a, uint8_t b, uint8_t c,
  uint8_t sclk, uint8_t latch, uint8_t oe, uint8_t dbuf) {
 
  RGBmatrixPanel_init(rgbmp, 8, a, b, c, sclk, latch, oe, dbuf);
}

// Constructor for 32x32 panel:
void RGBmatrixPanel_Constructor_32x32(RGBmatrixPanel *rgbmp, 
  uint8_t a, uint8_t b, uint8_t c, uint8_t d,
  uint8_t sclk, uint8_t latch, uint8_t oe, uint8_t dbuf) {

  RGBmatrixPanel_init(rgbmp, 16, a, b, c, sclk, latch, oe, dbuf);

  // Init a few extra 32x32-specific elements:
  rgbmp->_d        = d;
  rgbmp->_addrdport = portOutputRegister(digitalPinToPort(d));
  rgbmp->_addrdpin  = digitalPinToBitMask(d);
}

void RGBmatrixPanel_begin(RGBmatrixPanel *rgbmp) {

  rgbmp->_backindex   = 0;                         // Back buffer
  rgbmp->_buffptr     = rgbmp->_matrixbuff[1 - rgbmp->_backindex]; // -> front buffer
  activePanel = rgbmp;                      // For interrupt hander

  // Enable all comm & address pins as outputs, set default states:
  pinMode(rgbmp->_sclk , OUTPUT); SCLKPORT   &= ~rgbmp->_sclkpin;  // Low
  pinMode(rgbmp->_latch, OUTPUT); *rgbmp->_latport   &= ~rgbmp->_latpin;   // Low
  pinMode(rgbmp->_oe   , OUTPUT); *rgbmp->_oeport    |= rgbmp->_oepin;     // High (disable output)
  pinMode(rgbmp->_a    , OUTPUT); *rgbmp->_addraport &= ~rgbmp->_addrapin; // Low
  pinMode(rgbmp->_b    , OUTPUT); *rgbmp->_addrbport &= ~rgbmp->_addrbpin; // Low
  pinMode(rgbmp->_c    , OUTPUT); *rgbmp->_addrcport &= ~rgbmp->_addrcpin; // Low
  if(rgbmp->_nRows > 8) {
    pinMode(rgbmp->_d  , OUTPUT); *rgbmp->_addrdport &= ~rgbmp->_addrdpin; // Low
  }
  ...
  //TCCR1A  = 0x00;
  ////TCCR1B  = _BV(WGM13) | _BV(WGM12) | _BV(CS12); //CLKio/256 prescaler CTC mode
  //TCCR1B  = _BV(WGM13) | _BV(WGM12) | _BV(CS10); //CLKio/1 prescaler CTC mode
  
  //LIBRARY DEFAULTS MESSES UP ATMEGA32 MUSIC
  TCCR1A  = _BV(WGM11); // Mode 14 (fast PWM), OC1A off
  TCCR1B  = _BV(WGM13) | _BV(WGM12) | _BV(CS10); // Mode 14, no prescale Fast PWM mode
  
  ICR1    = 100;
  TIMSK  |= _BV(TOIE1); // Enable Timer interrupt
...
// 8/8/8 -> gamma -> 5/6/5
uint16_t RGBmatrixPanel_Color888Ex(RGBmatrixPanel *rgbmp, 
  uint8_t r, uint8_t g, uint8_t b, uint8_t gflag) {
...
uint16_t RGBmatrixPanel_ColorHSV(RGBmatrixPanel *rgbmp, 
  long hue, uint8_t sat, uint8_t val, uint8_t gflag) {
...
void RGBmatrixPanel_invertDisplay(Adafruit_GFX *agfx, uint8_t i) {}

void RGBmatrixPanel_drawPixel(Adafruit_GFX *agfx, int16_t x, int16_t y, uint16_t c) {
  uint8_t r, g, b, bit, limit, *ptr;
  RGBmatrixPanel *rgbmp = (RGBmatrixPanel*)agfx;
  if((x < 0) || (x >= rgbmp->agfx._width) || (y < 0) || (y >= rgbmp->agfx._height)) return;

  switch(agfx->_rotation) {
   case 1:
    swap(x, y);
    x = rgbmp->agfx._WIDTH  - 1 - x;
    break;
   case 2:
    x = rgbmp->agfx._WIDTH  - 1 - x;
    y = rgbmp->agfx._HEIGHT - 1 - y;
    break;
   case 3:
    swap(x, y);
    y = rgbmp->agfx._HEIGHT - 1 - y;
  ...
  // Loop counter stuff
  bit   = 2;
  limit = 1 << rgbmp->_nPlanes;

  if(y < rgbmp->_nRows) {
    // Data for the upper half of the display is stored in the lower
    // bits of each byte.
    ptr = &rgbmp->_matrixbuff[rgbmp->_backindex][y * rgbmp->agfx._WIDTH * (rgbmp->_nPlanes - 1) + x]; // Base addr
    // Plane 0 is a tricky case -- its data is spread about,
    // stored in least two bits not used by the other planes.
    ptr[64] &= ~0x03;            // Plane 0 R,G mask out in one op
    if(r & 1) ptr[64] |=  0x01;  // Plane 0 R: 64 bytes ahead, bit 0
    if(g & 1) ptr[64] |=  0x02;  // Plane 0 G: 64 bytes ahead, bit 1
    if(b & 1) ptr[32] |=  0x01;  // Plane 0 B: 32 bytes ahead, bit 0
    else      ptr[32] &= ~0x01;  // Plane 0 B unset; mask out
    // The remaining three image planes are more normal-ish.
    // Data is stored in the high 6 bits so it can be quickly
    // copied to the DATAPORT register w/6 output lines.
    for(; bit < limit; bit <<= 1) {
      *ptr &= ~0x1c;             // Mask out R,G,B in one op
      if(r & bit) *ptr |= 0x04;  // Plane N R: bit 2
      if(g & bit) *ptr |= 0x08;  // Plane N G: bit 3
      if(b & bit) *ptr |= 0x10;  // Plane N B: bit 4
      ptr  += rgbmp->agfx._WIDTH;                  // Advance to next bit plane
    }
  } else {
    // Data for the lower half of the display is stored in the upper
    // bits, except for the plane 0 stuff, using 2 least bits.
    ptr = &rgbmp->_matrixbuff[rgbmp->_backindex][(y - rgbmp->_nRows) * rgbmp->agfx._WIDTH * (rgbmp->_nPlanes - 1) + x];
    *ptr &= ~0x03;               // Plane 0 G,B mask out in one op
    if(r & 1)  ptr[32] |=  0x02; // Plane 0 R: 32 bytes ahead, bit 1
    else       ptr[32] &= ~0x02; // Plane 0 R unset; mask out
    if(g & 1) *ptr     |=  0x01; // Plane 0 G: bit 0
    if(b & 1) *ptr     |=  0x02; // Plane 0 B: bit 0
    for(; bit < limit; bit <<= 1) {
      *ptr &= ~0xe0;             // Mask out R,G,B in one op
      if(r & bit) *ptr |= 0x20;  // Plane N R: bit 5
      if(g & bit) *ptr |= 0x40;  // Plane N G: bit 6
      if(b & bit) *ptr |= 0x80;  // Plane N B: bit 7
      ptr  += rgbmp->agfx._WIDTH;                  // Advance to next bit plane
    }
  }
}

void RGBmatrixPanel_fillScreen(Adafruit_GFX *agfx, uint16_t c) {
	RGBmatrixPanel *rgbmp = (RGBmatrixPanel*)agfx;
  if((c == 0x0000) || (c == 0xffff)) {
    // For black or white, all bits in frame buffer will be identically
    // set or unset (regardless of weird bit packing), so it's OK to just
    // quickly memset the whole thing:
    memset(rgbmp->_matrixbuff[rgbmp->_backindex], c, 32 * rgbmp->_nRows * 3);
  } else {
    // Otherwise, need to handle it the long way:
    Adafruit_GFX_fillScreen(agfx, c);
  }
}

// Return address of back buffer -- can then load/store data directly
uint8_t *RGBmatrixPanel_backBuffer(RGBmatrixPanel *rgbmp) {
  return rgbmp->_matrixbuff[rgbmp->_backindex];
}
...
void RGBmatrixPanel_swapBuffers(RGBmatrixPanel *rgbmp, uint8_t copy) {
  if(rgbmp->_matrixbuff[0] != rgbmp->_matrixbuff[1]) {
    // To avoid 'tearing' display, actual swap takes place in the interrupt
    // handler, at the end of a complete screen refresh cycle.
    rgbmp->_swapflag = 1;                  // Set flag here, then...
    while(rgbmp->_swapflag == 1) wait_avr_ms(1); // wait for interrupt to clear it //TODO: check delay(1) replacement
    if(copy == 1)
      memcpy(rgbmp->_matrixbuff[rgbmp->_backindex], rgbmp->_matrixbuff[1-rgbmp->_backindex], 32 * rgbmp->_nRows * 3);
  }
}
...
void RGBmatrixPanel_dumpMatrix(RGBmatrixPanel *rgbmp) {
  ...
  //disabled we don't have Serial Class
  /*
  int i, buffsize = 32 * rgbmp->_nRows * 3;
  ...
  for(i=0; i<buffsize; i++) {
    Serial.print("0x");
    if(rgbmp->_matrixbuff[rgbmp->_backindex][i] < 0x10) Serial.print('0');
    Serial.print(rgbmp->_matrixbuff[rgbmp->_backindex][i],HEX);
    if(i < (buffsize - 1)) {
